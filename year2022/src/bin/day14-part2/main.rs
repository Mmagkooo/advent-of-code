...
fn put_sand(matrix: &mut Matrix) -> Point {
    ...
    loop {
        ...
        let down_element = matrix.get(&down_point).cloned();
...
fn main() {
    ...
    for x in -1000..1000 {
        matrix.insert(Point { x, y: bottom }, Element::Rock);
    }
    ...
    let mut steps = 0;
    loop {
        let sand_point = put_sand(&mut matrix);
        steps += 1;
        if sand_point == sand_source {
            break;
        }

...
fn get_max_geode(
    ...
    if (time <= 6 && factory.geode_robots == 0)
        || (time <= 7 && factory.obsidian_robots == 0)
        || (time <= 8 && factory.clay_robots == 0)
    {
        return i32::MIN;
    }
...
fn main() {
    ...
    let blueprints: Vec<Blueprint> = stdin()
        ...
        .take(3) // taking only the first 3 blueprints
        .map(|line| {
            let caps: Vec<i32> = re
                .captures_iter(&line)
    ...
    // faster execution, with a higher memory use, could be achieved with parallelization, as in part1

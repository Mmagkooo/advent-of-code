...
const MAX_TIME: i32 = 387; // heuristic
...
fn step(
    ...
    let config: Config = (expedition, time_modulo);
    ...
    // check if end
    if expedition == (field.height - 1, field.width - 1) {
        println!("Reached end in {}", time + 1);
        return;
    }
    ...
    for direction in [(0, 0), UP, RIGHT, DOWN, LEFT] {
        ...
        if is_in_bounds(&new_expedition, &new_field) {
            step(&new_field, new_expedition, time + 1, memo, blizzard_memo);
        }
    }
}
...
fn main() {
    ...
    step(&field, (-1, 0), 0, &mut memo, &mut blizzard_memo);

    // end was reached on multiple occassions, need to find quickest one
    let min_time = memo
        .iter()
        .map(|((point, _), value)| (point, value))
        .filter(|(point, _)| **point == (height - 1, width - 1))
        .map(|(_, value)| value)
        .min()
        .unwrap();
    println!("{}", min_time + 1); // +1 because time is measured until (height-1, width-1), but end is one place under
}

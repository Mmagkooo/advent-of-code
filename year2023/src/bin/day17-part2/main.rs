...
const MIN_STRAIGHT_STEPS: u32 = 4;
...
#[inline]
fn is_end(state: &State, height: isize, width: isize) -> bool {
    state.location.0 == height - 1
        && state.location.1 == width - 1
        && state.straight_steps >= MIN_STRAIGHT_STEPS
        && state.straight_steps <= MAX_STRAIGHT_STEPS
}
...
fn search(grid: &Grid) -> u32 {
    ...
    }) = queue.pop()
    {
        ...
        if is_end(&current_state, height, width) {
        ...
        if current_state.straight_steps < MAX_STRAIGHT_STEPS {
            next_directions.push((
                current_state.direction.clone(),
                current_state.straight_steps + 1,
            ));
        }

        if current_state.straight_steps >= MIN_STRAIGHT_STEPS
            && current_state.straight_steps <= MAX_STRAIGHT_STEPS
        {
            next_directions.push((current_state.direction.turn_left(), 1));
            next_directions.push((current_state.direction.turn_right(), 1));
        }

...
impl Direction {
    ...
    fn from_char(c: char) -> Self {
        match c {
            ...
            '<' => Self::LEFT,
            '>' => Self::RIGHT,
            other => panic!("Invalid direction char: {other}"),
        }
    }
}
...
fn calculate_next_locations(
    ...
    for direction in Direction::ALL {
        ...
        if next_char != EMPTY {
            let required_direction = Direction::from_char(next_char);
...
fn to_graph(input_chars: &Vec<Vec<char>>, end_location: Location) -> Graph {
    ...
    while !queue.is_empty() {
        ...
        let mut distance = 1;
        let mut one_way = false; // will be true if arrow encountered
        ...
        loop {
            ...
            match next_locations.len() {
                ...
                1 => {
                    ...
                    let next_char = input_chars[next_location.0 as usize][next_location.1 as usize];
                    if next_char != EMPTY {
                        // is arrow, direction ensured by calculate_next_locations
                        one_way = true;
                    }
                    ...
                    if let Some(old_distance) = neighbors_to_original.get(&next_location) {
                        if distance > *old_distance {
                            ...
                            if one_way {
                                // it's arrow, and it's a correct one - ensured by calculate_next_locations
                                graph
...
fn main() {
    ...
    // since history will be in the form of a 32-bit number
    assert!(graph.len() < 32);

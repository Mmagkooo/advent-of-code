...
while len(carts_set) > 1:
    ...
    for index, cart in enumerate(carts):
        ...
        if ntuple in carts_set:
            carts_set.remove(ntuple)
            for index, cart in enumerate(carts):
                if cart is None:
                    continue
                row, col, c, cnt = cart
                if (row, col) == ntuple:
                    carts[index] = None
                    break
        elif ntuple in ncarts_set:
            ncarts_set.remove(ntuple)
            for index, cart in enumerate(ncarts):
                if cart is None:
                    continue
                row, col, c, cnt = cart
                if (row, col) == ntuple:

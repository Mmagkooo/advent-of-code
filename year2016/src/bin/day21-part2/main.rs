...
fn scramble(password: &Vec<char>, commands: &Vec<String>) -> Vec<char> {
...
fn main() {
    ...
    let password: Vec<char> = argv[1].chars().collect();

    for candidate in password.iter().permutations(password.len()) {
        let mut candidate: Vec<char> = candidate.iter().map(|c| **c).collect();
        let scrambled = scramble(&mut candidate, &lines);
        if scrambled == password {
            let original_password: String = candidate.iter().collect();
            println!("{}", original_password);
        }
    }
}
